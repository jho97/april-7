...
sayHelloButton.addEventListener('cicked', sayHello)
...
const request = () => {
    axios.get('http://localhost:3000/query-test?name=John&age=24')
        .then(res => {
            console.log(res.data)
        })
...
queryBtn.addEventListener('click', request)
...
/*
    ...
    1: Send no queries on the URL -- what happened? I got a message that stated I had name and it's value was Jonathan. 

    2: Send more than 1 query on the URL -- what happened? It said that I sent more than 1 query and then listed the queries below the message
*/
...
//     if (obj.newFood === null || obj.newFood != "") {
//         axios.post('http://localhost:3000/food', obj)
//         .then(res => {
//             let x = document.createElement('li');
//             x.textContent = res.data[res.data.length -1];
